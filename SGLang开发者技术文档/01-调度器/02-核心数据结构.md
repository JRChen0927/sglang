...
**从ScheduleBatch到ForwardBatch的转换**：

```python
# 数据转换的核心流程（简化版）
def create_forward_batch(schedule_batch: ScheduleBatch, 
                        model_worker_batch: ModelWorkerBatch) -> ForwardBatch:
    """创建GPU前向计算批次"""
    return ForwardBatch(
        forward_mode=model_worker_batch.forward_mode,
        batch_size=len(schedule_batch.reqs),
        input_ids=model_worker_batch.input_ids,        # 从ModelWorkerBatch获取
        ...
        # 从调度器获取资源引用
        token_to_kv_pool=schedule_batch.token_to_kv_pool_allocator.get_kv_cache(),
