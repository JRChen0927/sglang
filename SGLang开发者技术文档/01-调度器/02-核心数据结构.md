...
|----------|--------|----------|----------|------------|
| **基础信息** | `rid` | `str` | 请求唯一标识符，用于全链路追踪 | [schedule_batch.py#L439](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L439) |
| | `origin_input_text` | `str` | 原始输入文本，保留用户原始输入 | [schedule_batch.py#L440](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L440) |
| | `origin_input_ids` | `List[int]` | Token化后的输入序列 | [schedule_batch.py#L446](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L446) |
| | `output_ids` | `List[int]` | 模型生成的输出token序列 | [schedule_batch.py#L448](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L448) |
| **处理配置** | `sampling_params` | `SamplingParams` | 采样策略配置（温度、top-p等） | [schedule_batch.py#L461](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L461) |
| | `stream` | `bool` | 是否启用流式输出，影响结果返回方式 | [schedule_batch.py#L423](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L423) |
| | `return_logprob` | `bool` | 是否返回对数概率，用于置信度分析 | [schedule_batch.py#L420](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L420) |
| | `lora_id` | `Optional[str]` | LoRA适配器标识，支持多模型切换 | [schedule_batch.py#L425](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L425) |
| **状态管理** | `finished_reason` | `Optional[str]` | 请求完成原因（长度限制/停止词/错误） | [schedule_batch.py#L466](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L466) |
| | `to_abort` | `bool` | 是否需要中止，用于请求取消 | [schedule_batch.py#L467](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L467) |
| | `req_pool_idx` | `Optional[int]` | 在内存池中的索引位置 | [schedule_batch.py#L468](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L468) |
| **多模态** | `multimodal_inputs` | `Optional[MultimodalInputs]` | 图像/视频/音频等非文本输入 | [schedule_batch.py#L452](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L452) |
| | `input_embeds` | `Optional[Tensor]` | 预计算的输入嵌入向量 | [schedule_batch.py#L426](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L426) |
| | `token_type_ids` | `Optional[Tensor]` | 跨编码器模型的token类型标识 | [schedule_batch.py#L455](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L455) |
| **会话支持** | `session_id` | `Optional[str]` | 会话标识，支持连续对话 | [schedule_batch.py#L451](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L451) |
| | `bootstrap_host` | `Optional[str]` | 分离式推理的启动主机地址 | [schedule_batch.py#L432](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L432) |
| | `bootstrap_port` | `Optional[int]` | 分离式推理的启动端口号 | [schedule_batch.py#L433](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L433) |
| | `bootstrap_room` | `Optional[str]` | 分离式推理的房间ID，用于请求路由 | [schedule_batch.py#L434](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L434) |
| **性能优化** | `prefix_indices` | `torch.Tensor` | 前缀缓存命中的token索引，减少重复计算 | [schedule_batch.py#L500+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L500) |
| | `extend_input_len` | `int` | 需要预填充的token数量，用于分块处理 | [schedule_batch.py#L500+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L500) |
| | `surr_offset` | `Optional[int]` | 环绕偏移量，用于滑动窗口注意力优化 | [schedule_batch.py#L500+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L500) |
| **分离式架构** | `disagg_kv_sender` | `Optional[BaseKVSender]` | KV缓存发送器，用于预填充/解码分离 | [schedule_batch.py#L600+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L600) |
| | `disagg_kv_receiver` | `Optional[BaseKVReceiver]` | KV缓存接收器，接收远程KV数据 | [schedule_batch.py#L600+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L600) |
| | `swa_uuid_for_lock` | `Optional[str]` | SWA混合缓存的锁定UUID，防止并发冲突 | [schedule_batch.py#L600+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L600) |
...
| 字段类别 | 字段名 | 继承关系 | 业务含义说明 | GitHub源码 |
|----------|--------|----------|--------------|------------|
| **核心组织** | `reqs` | 新增字段 | 批次中的Req对象列表，是批次的核心数据 | [schedule_batch.py#L819](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L819) |
| | `req_to_token_pool` | 新增字段 | 请求到token池的映射管理器，调度器资源管理 | [schedule_batch.py#L820](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L820) |
| | `token_to_kv_pool_allocator` | 新增字段 | KV缓存分配器，管理GPU内存分配和回收 | [schedule_batch.py#L821](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L821) |
| | `tree_cache` | 新增字段 | 前缀缓存树，优化重复计算和内存使用 | [schedule_batch.py#L822](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L822) |
| **批次控制** | `forward_mode` | 继承到下层 | 前向模式，控制批次执行模式（预填充/解码/混合） | [schedule_batch.py#L827](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L827) |
| | `batch_is_full` | 调度器特有 | 批次满标志，用于调度决策，不传递到下层 | [schedule_batch.py#L832](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L832) |
| | `enable_overlap` | 继承到下层 | 重叠执行标志，影响GPU-CPU并行策略 | [schedule_batch.py#L828](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L828) |
| | `launch_done` | 调度器特有 | 线程同步事件，用于重叠调度，不传递到下层 | [schedule_batch.py#L835](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L835) |
| **GPU数据** | `input_ids` | 继承到下层 | 输入token ID张量，从Req聚合而成的批量格式 | [schedule_batch.py#L845](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L845) |
| | `seq_lens` | 继承到下层 | 序列长度张量，用于注意力掩码计算 | [schedule_batch.py#L849](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L849) |
| | `req_pool_indices` | 继承到下层 | 请求池索引张量，用于内存映射 | [schedule_batch.py#L848](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L848) |
| | `out_cache_loc` | 继承到下层 | KV缓存输出位置，指向GPU内存地址 | [schedule_batch.py#L851](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L851) |
| **高级功能** | `spec_algorithm` | 继承到下层 | 投机解码算法类型（Eagle/Medusa等） | [schedule_batch.py#L900+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L900) |
| | `can_run_dp_cuda_graph` | 调度器特有 | 是否可运行DP CUDA图优化，调度决策专用 | [schedule_batch.py#L900+](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L900) |
| | `hicache_consumer_index` | 继承到下层 | 分层缓存消费者索引，用于CPU-GPU数据同步 | [schedule_batch.py#L914](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L914) |
| **特殊功能** | `has_stream` | 批次特性 | 批次中是否包含流式请求，影响输出处理 | [schedule_batch.py#L948](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L948) |
| | `has_grammar` | 批次特性 | 批次中是否包含语法约束请求 | [schedule_batch.py#L949](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L949) |
| | `is_prefill_only` | 批次特性 | 是否为纯预填充批次（max_new_tokens=0） | [schedule_batch.py#L953](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L953) |
| | `chunked_req` | 调度器特有 | 当前分块请求引用，用于超长序列处理 | [schedule_batch.py#L838](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/managers/schedule_batch.py#L838) |
