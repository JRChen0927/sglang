...
**指标收集系统的设计**：SchedulerMetricsMixin通过统一的指标收集接口，提供吞吐量、延迟、资源使用等关键性能指标的实时监控，支持多维度的性能分析。

> 📝 **简化说明**：以下为指标收集的核心概念简化版本，突出主要指标类型。真实实现包含投机解码、步骤时间等更多细粒度指标。
...
class SchedulerMetricsMixin:
    ...
    def init_metrics(self, tp_rank: int, pp_rank: int, dp_rank: Optional[int]):
        # 核心性能指标
        self.last_gen_throughput: float = 0.0      # 生成吞吐量
        self.last_input_throughput: float = 0.0    # 输入吞吐量
        self.stats = SchedulerStats()              # 统计信息
...
💡 **实现说明**: 真实的指标系统包含吞吐量、投机解码、内存使用、缓存命中率等10+种指标，支持Prometheus集成和分布式监控。教学版本突出"吞吐量→统计→收集器"的核心架构。
```
...
```python
def log_prefill_stats(self, can_run_list: List[Req], running_bs: int):
    """预填充统计的核心概念"""
    # 1. 计算关键指标
    ...
    # 2. 记录日志
    msg = f"#queue_req: {len(self.waiting_queue)}, #can_run_req: {num_can_run_reqs}"
    logger.info(msg)
```
...
💡 **实现说明**: 真实的统计系统包含队列长度、可运行请求数、token使用率、内存利用率等多维度指标，支持预填充和解码两个阶段的详细统计。
```
