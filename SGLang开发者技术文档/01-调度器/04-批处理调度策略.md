...
**批次调度的核心逻辑**：get_next_batch_to_run方法是SGLang批处理调度的核心，负责管理批次状态转换、处理分块请求、合并批次和获取新预填充批次。

> 📝 **简化说明**：以下为批次调度的真实实现，展示了SGLang的完整批次管理逻辑。完整实现请参考 `sglang/srt/managers/scheduler.py`。
...
**预填充批次获取的完整逻辑**：get_new_batch_prefill方法负责从等待队列中智能选择请求，构建新的预填充批次。

> 📝 **简化说明**：以下展示真实的get_new_batch_prefill实现，包含语法队列、LoRA约束、分层缓存等复杂逻辑。完整实现请参考 `sglang/srt/managers/scheduler.py`。
...
def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
    ...
    new_batch = ScheduleBatch.init_new(
        can_run_list,                      # 可运行请求列表
        self.req_to_token_pool,           # 请求到token池
        self.token_to_kv_pool_allocator,  # KV缓存分配器
        self.tree_cache,                  # 前缀缓存
        self.model_config,                # 模型配置
        self.enable_overlap,              # 是否启用重叠
        self.spec_algorithm,              # 投机解码算法
        chunked_req=self.chunked_req,     # 分块请求
    )
...
**智能请求添加器**：PrefillAdder负责从等待队列中智能选择请求，基于token预算、前缀缓存、LoRA约束等多维度因素进行优化。

> 📝 **简化说明**：以下展示真实的PrefillAdder实现，包含复杂的预算管理和缓存优化。完整实现请参考 `sglang/srt/managers/schedule_policy.py`。
