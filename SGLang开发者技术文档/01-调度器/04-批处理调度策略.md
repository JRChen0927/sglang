...
```mermaid
flowchart LR
    A["开始:<br/>get_next_batch_to_run"] --> B{"存在<br/>chunked_req?"}
    B -->|是| C["缓存未完成请求<br/>释放内存槽位"]
    B -->|否| D{"存在last_batch<br/>且为EXTEND模式?"}
    C --> D
    
    D -->|是| E["过滤批次<br/>合并到running_batch"]
    D -->|否| F["获取新预填充批次"]
    E --> F
    
    F --> G["检查语法队列<br/>验证预填充条件"] 
    G --> H["创建PrefillAdder"]
    H --> I["前缀缓存匹配<br/>token预算检查"]
    ...
    L --> N["创建ScheduleBatch"]
    N --> O{"new_batch<br/>存在?"}
    O -->|是| P{"running_batch<br/>为空?"}
    O -->|否| Q["返回<br/>running_batch"]
    P -->|是| R["返回<br/>new_batch"]
    P -->|否| S["混合批次<br/>返回mixed_batch"]
    
    style A fill:#e3f2fd,color:#000000,stroke:#333
    style O fill:#fff3e0,color:#000000,stroke:#333
    style P fill:#fff3e0,color:#000000,stroke:#333
