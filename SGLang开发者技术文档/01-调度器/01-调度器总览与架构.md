...
> 📝 **简化说明**：以下为并行协调机制的概念性示例，展示核心同步逻辑。真实实现涉及复杂的通信协议和错误恢复机制。
...
**并行协调的实现方式**：
- 张量并行：通过 `self.tp_rank` 和 `self.tp_size` 确定当前节点在并行组中的位置
- 流水线并行：通过 `self.pp_rank` 和 `self.pp_size` 处理不同的模型层
- 数据并行：通过 `self.dp_rank` 和 `self.dp_size` 处理不同的请求批次  
- 专家并行：通过 `self.moe_ep_rank` 和 `self.moe_ep_size` 路由MoE专家计算
